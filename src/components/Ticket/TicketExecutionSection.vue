<template>
  <section class="ticket-execution-section">
    ...
    <div class="plan-list">
      ...
      >
        ...
        <div class="currency-field part-cost-input">
          ...
          <input
            :value="requiredPartCostInputs[item.id] ?? ''"
            type="text"
            inputmode="decimal"
            placeholder="0.00"
            @input="syncRequiredPartCostInput(item.id, ($event.target as HTMLInputElement).value)"
            @blur="formatRequiredPartCostInput(item.id)"
          />
        </div>
        <button type="button" class="ghost" @click="removeRequiredPartItem(index)">Remove</button>
      </div>
    </div>

    <button type="button" class="ghost" @click="addRequiredPartItem">Add Required Part</button>
    ...
    <div v-if="form.summaryOfWorkPerformedPhotos.length" class="photo-grid">
      <figure v-for="photo in form.summaryOfWorkPerformedPhotos" :key="photo.id" class="photo-card">
        <button
          type="button"
          class="photo-remove"
          title="Remove photo"
    ...
    <textarea
      :value="form.summaryOfWorkPerformed"
      rows="5"
      placeholder="Enter summary of work performed"
      @input="form.summaryOfWorkPerformed = ($event.target as HTMLTextAreaElement).value"
    />
    ...
    <label class="invoice-labor">
      ...
      <div class="currency-field">
        ...
        <input
          ...
          inputmode="decimal"
          placeholder="0.00"
          @input="syncLaborCostInput(($event.target as HTMLInputElement).value)"
    ...
    <div v-if="showCloseOutSections" class="section-heading">
      <h3>Summary of Further Recommendations</h3>
      <p>Document any recommendations for future service.</p>
    </div>

    <textarea
      v-if="showCloseOutSections"
      :value="form.summaryOfFurtherRecommendations"
      rows="5"
      placeholder="Enter summary of further recommendations"
      @input="form.summaryOfFurtherRecommendations = ($event.target as HTMLTextAreaElement).value"
    />
  </section>
</template>
...
const selectedPartsTotal = computed(() => {
  return props.form.requiredParts.reduce((total, item) => {
...
const normalizedLaborCost = computed(() => {
  const value = Number(props.form.laborCost ?? 0)
...
function makePlanItem(text = '', completed = false): PlanActionItem {
  return {
...
function makeRequiredPartItem(text = '', completed = false): RequiredPartItem {
  return {
    id: globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`,
    text,
    completed,
...
function addPlanItem() {
  props.form.planOfAction.push(makePlanItem())
}

function removePlanItem(index: number) {
  props.form.planOfAction.splice(index, 1)
}

function addRequiredPartItem() {
  const item = makeRequiredPartItem()
  props.form.requiredParts.push(item)
  requiredPartCostInputs.value[item.id] = formatCurrencyInputValue(item.cost)
}

function removeRequiredPartItem(index: number) {
  const [removed] = props.form.requiredParts.splice(index, 1)
  if (removed?.id) delete requiredPartCostInputs.value[removed.id]
}

function syncRequiredPartCostInput(partId: string, rawValue: string) {
  requiredPartCostInputs.value[partId] = sanitizeCurrencyInput(rawValue)
...
function syncLaborCostInput(rawValue: string) {
  laborCostInput.value = sanitizeCurrencyInput(rawValue)
  props.form.laborCost = parseCurrencyInputValue(laborCostInput.value)
}

function formatLaborCostInput() {
  laborCostInput.value = formatCurrencyInputValue(props.form.laborCost)
  props.form.laborCost = parseCurrencyInputValue(laborCostInput.value)
}

function hydrateRequiredPartCostInputs() {
  const next: Record<string, string> = {}
  props.form.requiredParts.forEach((item) => {
    next[item.id] = formatCurrencyInputValue(item.cost)
  })
  requiredPartCostInputs.value = next
}

function hydrateLaborCostInput() {
  laborCostInput.value = formatCurrencyInputValue(props.form.laborCost)
}

watch(
  () => props.form.requiredParts,
  () => {
...
watch(
  () => props.form.laborCost,
  () => {
    hydrateLaborCostInput()
  },
  { immediate: true },
)
